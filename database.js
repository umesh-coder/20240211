...
function rename_database(database_name, new_database_name) {
    fs.rename(database_name, new_database_name, (err) => {
        if (err) throw err
        console.log(`Database "${database_name}" renamed to "${new_database_name}" successfully.`)
    })
}
...
function create_table(database_name, table_name) {
    ...
    if (fs.existsSync(filePath)) {
        console.log("table is already exist", table_name, "database name", database_name)
    ...
    fs.writeFile(filePath, JSON.stringify(row), (err) => {
        if (err) throw err;
        console.log(`Table "${table_name}" created successfully in database "${database_name}".`);
    });
...
function delete_table(database_name, table_name) {
    ...
    fs.unlink(filePath, (err) => {
        if (err) throw err
        console.log(`Table "${table_name}" deleted successfully from database "${database_name}".`)
    })
...
function rename_table_name(database_name, old_table_name, new_table_name) {
    ...
    if (!fs.existsSync(oldFilePath)) {
        console.log(`Table "${old_table_name}" does not exist in database "${database_name}".`)
        return
    }
    ...
    fs.rename(oldFilePath, newFilePath, (err) => {
        if (err) throw err
        console.log(`Table "${old_table_name}" renamed to "${new_table_name}" in database "${database_name}" successfully.`)
    })
...
function create_row(database_name, table_name, record) {
    ...
    fs.readFile(path.join(database_name, table_name), 'utf8', (err, data) => {
        ...
        fs.writeFile(path.join(database_name, table_name), JSON.stringify(dataArray), (err) => {
            if (err) throw err
            console.log(`Data written to "${table_name}" in Database "${database_name}" successfully.`)
        });
    });
...
function update_row(database_name, table_name, id, update_record) {
    ...
    fs.readFile(filePath, 'utf8', (err, data) => {
        ...
        if (indexToUpdate === -1) {
            console.log(`Record with ID "${id}" not found in table "${table_name}" in database "${database_name}".`)
            return;
        }
        ...
        fs.writeFile(filePath, JSON.stringify(dataArray), (err) => {
            if (err) throw err
            console.log(`Record with ID "${id}" updated successfully in table "${table_name}" in database "${database_name}".`)
        })
    })
}
...
function delete_row(database_name, table_name, id) {
    ...
    fs.readFile(filePath, 'utf8', (err, data) => {
        ...
        fs.writeFile(filePath, JSON.stringify(dataArray), (err) => {
            if (err) throw err
            console.log(`Record with ID "${id}" deleted successfully from table "${table_name}" in database "${database_name}".`)
        });
    });
}
...
function jsontotext_table(databaseName, tableName) {
    fs.readFile(path.join(databaseName, tableName), 'utf8', (err, data) => {
        ...
        fs.writeFile(path.join(databaseName, `${tableName}_formatted.txt`), formattedTable, (err) => {
            if (err) throw err
                        ...
                        case 3:
                            readline.question('Enter DataBase Name:-', (db_name) => {
                                ...
                                readline.question(('Enter Table_Name.json:-'), (tb_name) => {
                                    ...
                                    readline.question(('Enter New_Table_Name.json:-'), (tb1_name) => {
...
function test() {
    ...
    // read_table( "umesh09" , "students_details.json")
    ...
    // create_row("database","text.json",{ id: 1, name: 'John Doe', dateOfBirth: '1990-01-01'})
    ...
    jsontotext_table("database1" , "text.json")
